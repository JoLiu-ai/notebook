...
4. **备份文件夹选择**：侧边栏内受浏览器限制无法弹出文件夹选择器，会在独立窗口中完成选择
5. **自动保存**：自动保存功能仅在编辑文本内容时触发，修改标题或网址需要手动保存
6. **PDF 导出**：PDF 导出使用浏览器打印功能，需要用户手动选择"另存为 PDF"
7. **DOCX 导出**：导出的 DOCX 文件实际上是 HTML 格式，可以在 Microsoft Word 中打开并另存为真正的 DOCX 格式
